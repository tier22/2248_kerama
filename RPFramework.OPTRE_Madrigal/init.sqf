...
if (isServer) then {
	...
	waitUntil {uiSleep 0; !(isNil {player}) && player == player && alive player};
	cutText ["Loading in...","BLACK",1];
	
	[player, false] remoteExec ["Server_fnc_initStats", 2];
	
	waitUntil {uiSleep 0; player getVariable ["loadedIn", false]};
	...
	waituntil {uiSleep 0; !(isNull (findDisplay 46))};
